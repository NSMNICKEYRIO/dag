...
def generate_dag_code(
    ...
    async_no_wait_node_ids = [node.id for node in ordered_nodes if node.execution_mode == "async_no_wait"]

    for idx, node in enumerate(ordered_nodes, start=1):
        group_id = node_map[node.id]["group_id"]
        ...
        group_code = dedent(
            f"""
            with TaskGroup(group_id=\"{group_id}\", tooltip=\"{node.name}\") as {group_id}:
                {group_id}_execute = PythonOperator(
                    task_id=\"execute\",
        ...
        group_defs.append(group_code)

        terminal_task_ref = f"{group_id}_execute"

        if node.branch_on_status:
            branch_task_id = f"branch_{sanitize_identifier(node.id)}"
            success_targets = [f"{node_map[target_id]['group_id']}_execute" for target_id in node.on_success_node_ids]
            failure_targets = [f"{node_map[target_id]['group_id']}_execute" for target_id in node.on_failure_node_ids]
            branch_code = dedent(
                f"""
    ...
    for node in root_nodes:
        deps.append(f"run_started_event >> {node_map[node.id]['group_id']}_execute")

    for parent_id, child_id in dependency_pairs:
        parent_ref = task_end_map[parent_id]
        child_ref = f"{node_map[child_id]['group_id']}_execute"
    ...
    joined_groups = "\n\n".join(indent(group, "    ") for group in group_defs)
    joined_branch_defs = "\n\n".join(branch_task_defs)
    joined_deps = "\n    ".join(deps)
    schedule_str = repr(schedule) if schedule else "None"
    group_ids_literal = repr(all_group_ids)
    async_node_ids_literal = repr(async_no_wait_node_ids)
...
FINAL_GROUP_IDS = {group_ids_literal}
ASYNC_NO_WAIT_NODE_IDS = {async_node_ids_literal}
...
def finalize_results(group_ids, async_node_ids, **context):
    ti = context["ti"]
    failed, success = [], []
    ...
    for gid in group_ids:
        ...
        async_meta = ti.xcom_pull(task_ids=execute_task_id, key="async_meta")
        if async_meta and async_meta.get("node_id"):
            async_meta_by_node[async_meta["node_id"]] = async_meta

    async_metas = []
    for node_id in async_node_ids:
        async_meta = async_meta_by_node.get(node_id)
        if async_meta:
...
{joined_branch_defs}
    ...
    finalize_results_task = PythonOperator(
        ...
        op_kwargs={{"group_ids": FINAL_GROUP_IDS, "async_node_ids": ASYNC_NO_WAIT_NODE_IDS}},
...
{
  "run_control_id": "invoice_pipeline_01",
  "triggerType": "O",
  "schedule": null,
  "nodes": [
    {
    ...
    {
      "id": "split_stage",
      "name": "Split Into Parallel Flows",
      "engine": "PYTHON",
      "executor_order_id": 3,
      "executor_sequence_id": 1,
      "execution_mode": "sync"
