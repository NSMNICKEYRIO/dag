...
        # Pull status directly from the task_id
        status = ti.xcom_pull(task_ids=t_id, key="task_status") or "FAILED"
        if status == "FAILED":
            failed.append(t_id)
        else:
            success.append(t_id)

        # Pull async metadata and store it using the task_id as the key
        async_meta = ti.xcom_pull(task_ids=t_id, key="async_meta")
        if async_meta and async_meta.get("node_id"):
            async_meta_by_task[t_id] = async_meta

    async_metas = []
    for t_id in async_task_ids:
        async_meta = async_meta_by_task.get(t_id)
        if async_meta:
            async_metas.append(async_meta)
        else:
            # We use t_id here because node_id isn't available if metadata is missing
            async_poll_failed.append({"task_id": t_id, "detail": "Missing async metadata"})
