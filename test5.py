    ...
    def validate_branch_references(self) -> "BuildDagPayload":
        ...
        for node in self.nodes:
            ...
            if not node.branch_on_status and referenced_ids:
                errors.append(
                    f"Node {node.id} has branch targets but branch_on_status=false"
                )
...
FIRE_AND_FORGET_NODE_IDS = {fire_and_forget_node_ids!r}
NODE_NAME_MAP = {node_name_map!r}
BRANCH_SKIP_WHITELIST = {sorted(branch_skip_whitelist)!r}
MERGE_NODE_IDS = {sorted(merge_node_ids)!r}
...
def finalize_results(node_ids, **context):
    ...
    conf = _get_conf(context)
    ti = context["ti"]
    task_instances_by_id = {{task_instance.task_id: task_instance for task_instance in dag_run.get_task_instances()}}
    ...
    for node_id in node_ids:
        task_instance = task_instances_by_id.get(node_id)
        state = str(task_instance.state).lower() if task_instance and task_instance.state else ""
        ...
        if node_id in FIRE_AND_FORGET_NODE_IDS:
            entry = {{
                "task_id": node_id,
                "name": node_label,
                "state": "submitted_no_track" if state == "success" else (state or "unknown"),
            ...
            if state == "success":
                ...
                unexpected_skipped.append({{**entry, "skip_type": "unexpected_skip"}})
            else:
                unknown.append(entry)
            continue

        if node_id in ASYNC_NODE_IDS and state == "success":
            async_entry, async_result = _check_async_status(node_id, node_label, payload, context, ti)
        ...
        entry = {{
            ...
            "status_source": "airflow_task_state",
        }}
