...
def generate_dag_code(
    ...
    for idx, node in enumerate(ordered_nodes, start=1):
        ...
        terminal_task_ref = f"{group_id}.execute"

        if node.branch_on_status:
            branch_task_id = f"branch_{sanitize_identifier(node.id)}"
            success_targets = [f"{node_map[target_id]['group_id']}.execute" for target_id in node.on_success_node_ids]
            failure_targets = [f"{node_map[target_id]['group_id']}.execute" for target_id in node.on_failure_node_ids]
            branch_code = dedent(
                f"""
    ...
    for node in root_nodes:
        deps.append(f"run_started_event >> {node_map[node.id]['group_id']}.execute")

    for parent_id, child_id in dependency_pairs:
        parent_ref = task_end_map[parent_id]
        child_ref = f"{node_map[child_id]['group_id']}.execute"
...
from airflow.sdk import DAG, TaskGroup, TriggerRule, get_current_context
